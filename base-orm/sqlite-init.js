//Crea la estructura de la base de datos y le carga datos
...
async function CrearBaseSiNoExiste() {
  ...
  let res = null;
  existe = false;
  //Se realiza una consulta SQL para verificar si existe una tabla llamada "articulos" en la base de datos. y si corresponde, se la crea.
  sql =
    "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= 'articulos'";
    //El WHERE es una cláusula de SQL para filtrar registros en una consulta de base de datos. Se usa con SELECT para especificar condiciones que deben cumplir los registros seleccionados.
  res = await db.get(sql, []);
  if (res.contar > 0) existe = true;
  //Si la tabla "articulos" no existe, se ejecuta una consulta SQL utilizando la función db.run para crear la tabla. La consulta crea una tabla llamada "articulos" con varias columnas
  if (!existe) {
    await db.run(
    ...
    // si se creo la tabla, se insertan un conjunto de registros
    await db.run(
